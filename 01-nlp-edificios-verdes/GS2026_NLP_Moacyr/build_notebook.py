...
cells = [
    ...
    md("## 8. Chatbot (Etapa 7)\n",
       "\n",
       "O chatbot conversacional (Streamlit + histórico de sessão + RAG opcional sobre o ",
       "ChromaDB) está em `app/chatbot_streamlit.py`, versionado no repo. Para demonstrar ",
       "no próprio Colab, suba via `streamlit` + um túnel (`cloudflared`/`pyngrok`). ",
       "Veja as instruções no `README.md` da pasta."),
    code(
        "# Demonstração opcional do chatbot no Colab (ver README para o passo a passo do túnel):\n",
